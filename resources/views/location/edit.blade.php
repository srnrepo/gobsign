			...
			<div class="col-sm-8">
				...
				<div class="box-info no-padding-rl padding-top0">
					<h2 class="col-sm-12 padding-top20 margin-top0">
						<strong>{!! trans('messages.Edit') !!}</strong> {!! trans('messages.Location') !!}
					...
					<div class="col-sm-9 col-xs-9 col-xs-offset-3 col-sm-offset-3 padding-l30 padding-top10">
						{{ App\Classes\Helper::getCustomFields('location-form',$custom_field_values) }}
						{!! Form::submit(isset($buttonText) ? $buttonText : trans('messages.Update Location'),['class' => 'btn btn-primary']) !!}
					</div>
				</div>
				{!! Form::close() !!}
			</div>
			<div class="col-sm-4">
				<div class="the-notes info"><h4>{!! trans('messages.Help') !!}</h4>Here you can edit the location name & its description. Keep in mind that location name cannot be same as location in another client.
